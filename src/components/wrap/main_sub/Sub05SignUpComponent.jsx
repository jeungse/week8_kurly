...
export default function Sub05SignUpComponent({ 회원가입 }) {
    ...
    const {
        isId, isIdDuplCheck, isPw1, isPw2, isName, isEmail, isEmailDuplCheck, isHp, isHpDis, isUserHpDis, isAddr1, isAddr2, 
        isGender, isChooga1, isChooga2, isService, msgAddr1, msgAddr2, msgChooga1,
        msgChooga2, msgEmail, msgGender, msgHp, msgId, msgName, msgPw1,
        msgPw2, msgService, msgBirth, 아이디, 비밀번호, 비밀번호확인, 이름, 이메일, 휴대폰, 휴대폰인증발급번호, 휴대폰인증입력번호,
        생년, 생월, 생일, 추천인아이디, 추가입력사항, choogaPlaceHolder, choogaGuideText, 이용약관, 이용약관동의, 체크아이디중복, 
        체크이메일중복, 체크휴대폰인증, 추천인아이디존재, 체크비밀번호확인
    } = state;
    ...
    const onClickDuplCheck = (e, value) => {
        ...
        if (value === 'ID중복확인') {
            ...
            axios({
                url:'http://saeeun.dothome.co.kr/kurly/id_deplicate_check.php',
                method:'post',
                data: newFormData
            })
            .then((res) => {
            ...
            axios({
                url:'http://saeeun.dothome.co.kr/kurly/email_deplicate_check.php',
                method:'post',
                data: newFormData
            })
            .then((res) => {
    ...
    // 생년월일
    // 생년 1923~2023
    // 1. 생년 > 올해년도 - 100 100세 이하 생년월일을 다시 입력하세요. 
    // 2. 생년 > 올해년도 생년월일이 미래로 입력되었습니다. 
    ...
    React.useEffect(()=>{
        // 세개의 입력값이 모두 없다면 => 하단에 오류메시지 모두 삭제
        if (생년 === '' && 생월 === '' && 생일 === '') {
            setState({
                ...state,
                msgBirth: ''
            ...
            if(생년 === ''){
                setState({
                    ...state,
                    msgBirth: '태어난 년도 4자리를 정확하게 입력하세요.',
                })
            } else {
                if (Number(생년) < new Date().getFullYear()-100) {
                    setState({
                        ...state,
                        msgBirth: '태어난 년도를 다시 확인해주세요(100세 이하).'
                    })
                    ...
                    setState({
                        ...
                        setState({
                            ...state,
                            msgBirth: '태어난 월을 입력하세요.',
                        })
                } else if ( Number(생월) < 1 || Number(생월) > 12) {
                    setState({
                        ...state,
                        msgBirth: '태어난 월을 정확하게 입력하세요.',
                    })
                } else {
                    if ( 생일 === '' ) {
                        setState({
                            ...state,
                            msgBirth: '태어난 일을 입력하세요.',
                        })
                    } else if ( Number(생일) < 1 || Number(생일) > 31) {
                        setState({
                            ...state,
                            msgBirth: '태어난 일을 정확하게 입력하세요.',
                        })
                    } else {
                        setState({
                            ...state,
    ...
    const onClickChoochunIdOK=(e)=>{
        ...
        let newFormData = new FormData();
            ...
            axios({
                url:'http://saeeun.dothome.co.kr/kurly/id_duplicate_check.php',
                method:'post',
                data: newFormData
            })
            .then((res) => {
                if(res.status===200){
                    if(res.data===1){
                       ...
                       confrimModalOpen('존재하는 아이디입니다. 친구초대 이벤트 참가 가능해요.', 1);
                    }
                    else{
                        confrimModalOpen('없는 아이디입니다.');     
                    }
                }
            })
            .catch((err) => {
    ...
    const onSubmitSignup=(e)=>{
        ...
        if( 아이디 === '' ) {
            confrimModalOpen('아이디를 입력해주세요');
        } else if ( 체크아이디중복 === false ) {
            confrimModalOpen('아이디 중복확인을 해주세요');
        } else if ( 비밀번호 === '' ) {
            confrimModalOpen('비밀번호를 입력해주세요');
        } else if ( 체크비밀번호확인 === false) {
            confrimModalOpen('동일한 비밀번호를 입력해주세요');
        } else if ( 이름 === '' ) {
            confrimModalOpen('이름을 입력해주세요');
        } else if ( 이메일 === '' ) {
            confrimModalOpen('이메일을 입력해주세요');
        } else if ( 체크이메일중복 === false ) {
            confrimModalOpen('이메일 중복확인을 해주세요');
        } else if ( 휴대폰 === '' ) {
            confrimModalOpen('휴대폰번호를 입력해주세요');
        } else if ( 체크휴대폰인증 === false ) {
            confrimModalOpen('휴대폰 인증을 해주세요.');
        } else if ( post.주소1 === '' ) {
            confrimModalOpen('주소를 입력해주세요');
        } else if ( post.주소2 === '' ) {
            confrimModalOpen('상세주소를 입력해주세요');
        } else if ( 이용약관동의필수 < 3 ) {
            confrimModalOpen('필수이용약관에 동의해주세요');
        } else {
            ...
            axios({
                url: 'http://saeeun.dothome.co.kr/kurly/signup.php',
                method: 'POST',
                data: newFormData
            })
            .then((res)=>{
                            ...
                            <ul>
                                ...
                                <li className="row6">
                                    <div className='box1'>
                                        ...
                                        <input type="text" name='userHp' id='userHp' maxLength={11} onChange={(e) => onChangeEvent(e, '휴대폰')} value={휴대폰} placeholder='숫자만 입력해주세요' />
                                        <button disabled={!isHpDis} className={isHp ? '' : 'hp-btn'} onClick={onClickHpBtn}>{isUserHpDis ? '다른번호인증' : '인증번호받기'}</button>
                                    </div>
                                    <div className='box2'>
                                        {isHp && <p>{msgHp}</p>}
                                    </div>
                                ...
                                <li className="row8">
                                    <div className='box1'>
                                        ...
                                        {
                                            post.주소1 !== '' &&
                                            <input type="text" name='userAddr1' id='userAddr1' maxLength={250} onChange={onChangeAddr1Event} value={post.주소1} placeholder='검색주소' />
                                        }
                                        <button onClick={onClickAdd} className={post.주소1 === '' ? 'on' : ''}>{post.주소1 !== '' ? "재검색" : "주소검색"}</button>
                                    </div>
                                </li>
                                {
                                    post.주소1 !== '' &&
                                ...
                                <li className="row11">
                                    <div className='box1'>
                                        ...
                                        <div className='gender'>
                                            ...
                                            <label><input type="radio" onChange={onChangeGender} checked={state.성별.includes('선택안함')} name='none' id='none' defaultChecked={true} value="선택안함" /><em>선택안함</em></label>
                                        </div>
                                    </div>
                                </li>
                                <li className="row12">
                                    <div className='box1'>
                                        ...
                                        <div className='birth'>
                                            ...
                                            <label><input type="text" onChange={onChangeBirthDate} name='date' id='date' defaultChecked={true} value={생일} placeholder='DD' /></label>
                                        </div>
                                    </div>
                                    <div className='box2'>
                                        {<p>{msgBirth}</p>}
                                ...
                                {추가입력사항 !== "" &&
                                    (<>
                                        <li className="row14">
                                            <div className='box1'>
                                                <input type="text" name='userChoochun' id='userChoochun' maxLength={16} onChange={(e) => onChangeChoochunEvent(e)} value={추천인아이디} placeholder={choogaPlaceHolder} />
